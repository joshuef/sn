...
impl Dispatcher {
    pub(crate) async fn start_network_probing(self: Arc<Self>) -> Result<()> {
    ...
    pub(crate) async fn start_section_probing(self: Arc<Self>) -> Result<()> {
    ...
    pub(crate) async fn start_cleaning_peer_links(self: Arc<Self>) -> Result<()> {
    ...
    pub(crate) async fn start_sending_any_data_batches(self: Arc<Self>) -> Result<()> {
            ...
            if let Some(address) = this_batch_address {
                    ...
                    self.pending_data_to_replicate_to_peers.remove(&address)
                {
                        ...
                        .await
                    {
                        Ok(data) => data,
                        Err(error) => {
                            error!("Error getting data from storage in replication loop. {error}");
                            continue;
                        }
                    };
                    ...
                    let wire_msg =
                        match WireMsg::single_src(our_info, dst, system_msg, src_section_pk) {
                            Ok(res) => res,
                            Err(error) => {
                                error!("Error in wiremsg conversion {error}");
                                continue;
                            }
                        };
                    ...
                    cmds.extend(
                        match self.send_msg(&recipients, recipients.len(), wire_msg).await {
                            Ok(cmd) => cmd,
                            Err(error) => {
                                error!("Error sending msg in replication loop. {error}");
                                // extend with nothing
                                vec![]
                            }
                        },
    ...
    pub(crate) async fn check_for_dysfunction_periodically(self: Arc<Self>) -> Result<()> {
